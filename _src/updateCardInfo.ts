...
const REMOTE_DATA_PATH =    new URL('https://db.ygoprodeck.com/api/v7/cardinfo.php');
const REMOTE_VERSION_PATH = new URL('https://db.ygoprodeck.com/api/v7/checkDBVer.php');


let currentVersion;
try {
    currentVersion = (JSON.parse(await readFile(VERSION_PATH, 'utf-8')) as YpdCardInfoVersionRoot)[0].database_version;
} catch(e: any){
...
let remoteVersion;
try {
    versionResponse = await (await fetch(REMOTE_VERSION_PATH)).json() as YpdCardInfoVersionRoot;
...
console.log(`local version: v${currentVersion}\nYGOProDeck version: v${remoteVersion}`);
...
try {
     cardInfoResponse = await (await fetch(REMOTE_DATA_PATH)).json() as YpdCardInfoRoot;
     console.log(cardInfoResponse.data[0])
} catch(e: any){
...
cardInfoResponse.data.filter((card) => {
...
await git.commit({fs, dir: '.', message:`Update card data to ${versionResponse[0].last_update}`, author: {name: 'updater', email: 'no-reply@github.com'}});
...
let pushResult = await git.push({fs, http, dir: '.', remote: 'origin', 
